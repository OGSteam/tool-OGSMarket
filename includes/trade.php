...
//Annulation de la réservation	
 ...
 //Fin de la transaction et Archivage de l'offre.	
	function close_trade($tradeid){
		...
		$out="Archivé";
		$sql = "Update ".TABLE_TRADE." "
		." set `trade_closed`=1 where `id`=".$tradeid;
    ...
    elseif ($action == "userclosedtrades")
      $sql .= "u.id = t.traderid  AND t.universid = v.id AND t.`trade_closed` = 1";
		...
		while(list($id,$traderid,$universid,$g,$offer_metal,$offer_crystal,$offer_deuterium,$want_metal,$want_crystal,$want_deuterium,$creation_date,$expiration_date,$note,$username,$avatar_link,$deliver,$refunding,$pos_user,$pos_date) = $db->sql_fetch_row($result)){
			...
			foreach ($deliver as $key=>$value) $newvalues["deliver"][$value] = 1;
			for ($i = 1; $i <= $g; $i++) if (!isset($newvalues["deliver"][$i]) || $newvalues["deliver"][$i] != 1) $newvalues["deliver"][$i] = 0;
			
			$newvalues["refunding"] = Array();
			$refunding = explode('_', $refunding);
			foreach ($refunding as $key=>$value) $newvalues["refunding"][$value] = 1;
			for ($i = 1; $i <= $g; $i++) if (!isset($newvalues["refunding"][$i]) || $newvalues["refunding"][$i] != 1) $newvalues["refunding"][$i] = 0;
		...
		if ($action == "uniquetrade") return $tradearray[0];
		else return $tradearray;
	}
...
global $db, $Users, $Trades, $user_data,$server_config;
...
function del_trade($tradeid){
	...
	if ($trade){
		...
		} else return "Cette offre ($tradeid) ne vous appartient pas, vous ne pouvez donc pas l'&eacute;ffacer";
	} else return "Je ne trouve pas d'offre correspondant au numero $tradeid";
}

//reactivation de l'offre
function reactive_trade($trade_id){
	global $db, $Trades, $server_config, $current_uni, $user_data;
...
function update_trade (){
	...
	if(!isset($pub_expiration_hours)) $pub_expiration_hours = 0;
	if ($user_data["is_active"] !=1 ) return "Impossible de modifer cette offre, l'utilisateur est inactif.";
	else {
